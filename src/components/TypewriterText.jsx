...
export const TypewriterHeading = ({ heading, onComplete, className = "" }) => {
    ...
    return (
        <h2 className={`font-serif text-4xl md:text-5xl leading-tight text-brand-brown min-h-[3.5rem] ${className}`}>
            {displayedHeading}
            <motion.span
...
export const FadeText = ({ show, children, className = "" }) => {
    return (
        <motion.div
            ...
            className={`font-sans text-xl text-brand-text opacity-80 ${className}`}
        >
            {children}
        </motion.div>
    );
